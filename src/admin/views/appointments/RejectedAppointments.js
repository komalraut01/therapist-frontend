...
const AdminRejectedAppointments = () => {
  const [data, setData] = useState([])
  const getData = async () => {
    const result = await getAdminAppointment('rejected');
    setData(result.data)
  }
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>Rejected Appointment List</strong>
            </CCardHeader>
            <CCardBody>
              <CTable>
                <CTableHead>
...
export default AdminRejectedAppointments